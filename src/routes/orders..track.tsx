...
const getOrder = createServerFn({ method: "GET" }).handler(async (ctx) => {
  ...
  const order = db.prepare(`
    SELECT o.*, t.store_name as tenantName,
           json_group_array(json_object('name', p.name, 'quantity', oi.quantity, 'price', oi.price)) as items_json
    FROM orders o
...
function TrackOrder() {
  ...
  const parsedItems = (() => {
    try {
      return JSON.parse(order.items_json || "[]").filter((i: any) => i.name);
    } catch { return []; }
  })();
      ...
      <main className="max-w-2xl mx-auto px-6 py-12 animate-fade-in">
        ...
        <div className="text-center mb-10">
          ...
          <p className="text-[var(--color-neutral-500)]">
            Order #{id.slice(0, 8)} · {order.tenantName}
          </p>
          <Badge
